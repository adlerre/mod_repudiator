...
//
// Adopted from https://lucidar.me/en/dev-c-cpp/sha-256-in-c-cpp/
//
...
static uint32_t step1(uint32_t e, uint32_t f, uint32_t g) {
    return (rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)) + ((e & f) ^ ((~e) & g));
}

static uint32_t step2(uint32_t a, uint32_t b, uint32_t c) {
    return (rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)) + ((a & b) ^ (a & c) ^ (b & c));
}

static void update_w(uint32_t *w, int i, const uint8_t *buffer) {
    int j;
    for (j = 0; j < 16; j++) {
        if (i < 16) {
            w[j] =
